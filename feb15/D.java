...
class Node{
   //public int [] C = new int[3]; 
   public long cnt[] = new long[4];
   public long str[][]=new long[4][4];   
	...
	*/
	...
	public static void main(String[] args) throws java.lang.Exception{
		//let_me_start 
			...
			int n = s.length();
			for(int i=1;i<=n;i++){
				if(s.charAt(i-1)=='c')num[i]=0;
				else if(s.charAt(i-1)=='h')num[i]=1;
				else if(s.charAt(i-1)=='e')num[i]=2;
				else if(s.charAt(i-1)=='f')num[i]=3;
			}
		    Node[] m = new Node[3*n+10000];//size = 2*n+1
			//for(int i=1;i<=4*n;i++)m[i] = new Node();
			...
			for(int Q=1;Q<=qu;Q++){
			  ...
			  String[] sa = s.split(" ");
			    ...
			    if(sa[0].charAt(0)=='c')p=0;
				if(sa[0].charAt(0)=='h')p=1;
				if(sa[0].charAt(0)=='e')p=2;
				if(sa[0].charAt(0)=='f')p=3;
			   
				
				if(sa[1].charAt(0)=='c')q=0;
				if(sa[1].charAt(0)=='h')q=1;
				if(sa[1].charAt(0)=='e')q=2;
				if(sa[1].charAt(0)=='f')q=3;

			  int l = Integer.parseInt(sa[2]);
			  int r = Integer.parseInt(sa[3]);
			  Node1 ans = query(1,1,n,m,num,l,r);
			  res = ans.c;
			  out.write(""+res+"\n");
...
public static void maketree(int node,int i,int j,Node[] m,int []num)throws Exception{
		...
		//logic
		for(int k=0;k<=3;k++)m[node].cnt[k]=m[2*node].cnt[k]+m[2*node+1].cnt[k];
		for(int k=0;k<=3;k++)for(int l=0;l<=3;l++)if(k!=l)m[node].str[k][l]=m[2*node].str[k][l]+m[2*node+1].str[k][l]+m[2*node].cnt[k]*m[2*node+1].cnt[l];;
		//for(int k=0;k<=3;k++)for(int l=0;l<=3;l++)m[node].str[k][l]+=m[2*node].cnt[k]*m[2*node+1].cnt[l];
		
   }
	
	
	public static Node1 query(int node,int l,int r,Node[] m,int []num,int i,int j)throws Exception{
		...
		Node1 arr1 = query(2*node,l,(l+r)/2,m,num,i,j);
	    Node1 arr2 = query(2*node+1,((l+r)/2)+1,r,m,num,i,j);
		...
		arr.a=arr1.a+arr2.a;
		arr.b=arr1.b+arr2.b;
		arr.c=arr1.c+arr2.c+ arr1.a*arr2.b;
		//for(int k=0;k<=3;k++)arr.cnt[k]=arr1.cnt[k]+arr2.cnt[k];
		//for(int k=0;k<=3;k++)for(int t=0;t<=3;t++)arr.str[k][t]=arr1.str[k][t]+arr2.str[k][t]+arr1.cnt[k]*arr2.cnt[t];
		//for(int k=0;k<=3;k++)for(int t=0;t<=3;t++)arr.str[k][t]+=arr1.cnt[k]*arr2.cnt[t];
		return arr;
	}
