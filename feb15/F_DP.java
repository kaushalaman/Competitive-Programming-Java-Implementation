...
public static void main(String[] args) throws java.lang.Exception{
		//let_me_start 
			BufferedWriter out = new BufferedWriter(new OutputStreamWriter(System.out),200000);
			String s = s();
			...
			for(int i=1;i<=n;i++){
			  ...
			  cnt[i][num[i]]++;
			  for(int k=0;k<=3;k++)for(int j=0;j<=3;j++)dp[i][k][j]=dp[i-1][k][j];
			  for(int k=0;k<=3;k++)dp[i][k][num[i]] += cnt[i-1][k];
			  dp[i][num[i]][num[i]]+=1;
			  
			}
			int qu=i();
			long res=0;int p=0,q=0;
			for(int Q=1;Q<=qu;Q++){
			  s = s();
			  String[] sa = s.split(" ");
			  ...
			  res = dp[r][p][q]-dp[l-1][p][q] - (cnt[l-1][p]) *(cnt[r][q]-cnt[l-1][q]);
			  out.write(""+res+"\n");
			 // out.flush();
...
//****************************** Utilities ***********************//
public static int f(String s ,int ans[],int in)throws Exception{
			int n = s.length();
			if(in==1){
			ans[0]=(s.charAt(0)-'A')+5;
			}else{
			 ans[0]=(s.charAt(0)-'A');
			}
			
			for(int i=1;i<n;i++){
			   if(s.charAt(i)==s.charAt(i-1)&&in==1){
			     in=2;
				 ans[i]=ans[i-1]-5;
				 if(ans[i]<0||ans[i]>9){in=-1;break;}
				 continue;
			   }else if(s.charAt(i)==s.charAt(i-1)&&in==2){
			    in=1;
				ans[i]=ans[i-1]+5;
				if(ans[i]<0||ans[i]>9){in=-1;break;}
				continue;
			   }
			   if(Math.abs(s.charAt(i)-s.charAt(i-1))==1&&in==1){
			     in=-1;break;
			   }
			   if(((Math.abs(s.charAt(i)-s.charAt(i-1))!=4)&&((Math.abs(s.charAt(i)-s.charAt(i-1))!=1)))&&in==2){
			    in=-1;
				break;
			   }
			   ans[i]=ans[i-1]+s.charAt(i)-s.charAt(i-1);
			   
			}
			return in;
}
 public static boolean ch(int arr[],int arr1[])throws Exception{
     for(int i=0;i<arr.length;i++){
	   
	   if(arr1[i]<arr[i])return false;
	   else if(arr1[i]>arr[i]) return true;
	 }
	 return true;
 }
...
class FastReader{
	...
	private byte[] buf = new byte[1000024];
	private int curChar;
