...
public static void main(String[] args) throws java.lang.Exception{
		...
		char[] ch1 = new char[100005];
		char[] ch2 = new char[100005];
		ch1[0]='+';ch2[0]='-';
		for(int i=1;i<=100000;i++){
		 if(ch1[i-1]=='+')ch1[i]='-';else ch1[i]='+';
		 if(ch2[i-1]=='+')ch2[i]='-';else ch2[i]='+';
		 }
		for(int t=1;t<=tests;t++){
			...
			//ch = s.toCharArray();
			ans = Math.min(f(ch,ch1,n),f(ch,ch2,n));
			out.write(""+ans+"\n");
		 }
...
//****************************** Utilities ***********************//
public static int f(char ch[] ,char ch1[] ,int n)throws Exception{
  int cnt =0;
  for(int i=1;i<=n;i++)if(ch[i]!=ch1[i])cnt++;
  return cnt;
}
