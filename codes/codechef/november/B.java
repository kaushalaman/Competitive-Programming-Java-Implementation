		 ...
		 for(int t = 1; t<= tests ;t++){
			...
			for(int i=0;i<len/2; i++){
			 if(ch[i]!=ch[len-1-i]){
			  char c = ch[i];
			  ch[i]='@';
			  boolean a = isPalindrome(ch);
			  ch[i]=c;
			  c=ch[len-1-i];
			  ch[len-1-i]='@';
			  boolean b = isPalindrome(ch);
			  ch[len-i-1] = c;
			  if(a|b){
			   out.write("YES"+"\n");
			   break;
...
//****************************** Utilities ***********************//
public static boolean isPalindrome(char[] ch)throws Exception{
	int len = ch.length;
	int st =0; int end = len-1;
	
	while(st<=end){
	 if(ch[st]=='@'){st++;continue;} 
	 if(ch[end]=='@'){end--;continue;}
