...
{	
	...
	*/
	...
	public static void main(String[] args) throws java.lang.Exception{
		//let_me_start 
		 long tests = l(); 
		 double Matrix[][] = new double[150][150];
		 ...
		 for(int t = 1; t<= tests ;t++){
			...
			//long Matrix[][] = {{1,0,3},{1,0,2},{0,5,0}};
		
		 double T [][] = new double [26][26] ;
		 double Identity [][] = new double [26][26] ;
		 for(int i=0;i<26;i++)Identity[i][i] = 1.0D;
		...
		while(exp>=1){
			...
			if(exp%2==0){
				...
				*/
				for(int a=0;a<26;a++){
						for(int b=0;b<26;b++){
									for(int k=0;k<26;k++){
												var = ( var + (Identity[a][k]*Matrix[k][b] ) );
									}
		 ...
		 double ans=0;
			for(int i=1;i<=n;i++){
			   String si = s();
			   if(s.length()!=si.length()){continue;}
			  
			   //double temp = 1.0D;
			   Integer x = hm.get(si);
			   Integer tt = new Integer("1");
			   if(x==null)hm.put(si,tt);
			    else continue;
			   
			  double temp = 1.0D;
			
			   for(int j =0;j<s.length();j++){
			    temp *= Identity[s.charAt(j)-'a'][si.charAt(j)-'a'];
			   }
			   ans+=temp;
			}
			out.write(""+ans+"\n");
