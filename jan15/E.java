	...
	*/
	...
	public static void main(String[] args) throws java.lang.Exception{
        ...
        for(int t=1;t<=tests;t++){
			...
			int rb []  = new int [n+1];
			for(int i=1;i<=n;i++){
			  ra[i] = 1+((int)(Math.random()*n))%(n-1);
			  rb[i] = ra[i]+((int)(Math.random()*n))%(n-ra[i]);
			  //rb[i]=n-ra[i];
			}
			...
			StringBuilder []s = new StringBuilder[371];
		    for(int i=1;i<=370;i++)s[i]=new StringBuilder();
			
			
			for(int i=1;i<=n;i++){
			  s[1].append(arr[i]);
			  s[2].append(arr[n-i+1]);
			}
			BigInteger[] a = new BigInteger[371];
			
			
			//BigInteger a[] = new BigInteger(s[1].toString());
			
			a[1] = new BigInteger(s[1].toString());
			a[2] = new BigInteger(s[2].toString());
			
			//out.write("a="+a.toString()+"b="+b.toString()+"c="+c.toString()+"\n");
			int k =i();
			
			BigInteger [] m = new BigInteger[k+1];
			for(int i=1;i<=k;i++){
			 int x =i();
			 m[i] = new BigInteger(x+"");			 
			}
			BigInteger ans[] = new BigInteger[371];
			for(int i=1;i<=370;i++)ans[i] = new BigInteger("0");
			for(int x=3;x<=370;x++){
				change(arr,ra,rb,n);
				for(int i=1;i<=n;i++)s[x].append(""+arr[i]);
				a[x] = new BigInteger(s[x].toString()); 
			}   
			//String s = new String();
			int min=1;
			for(int x=1;x<=370;x++){
				...
				for(int i=1;i<=k;i++){
						ans[x]=ans[x].add(a[x].mod(m[i]));
				}
				if(ans[x].compareTo(ans[min])<0){
				 min=x;
				}
			}
			out.write(""+s[min].toString()+"");				
...
//****************************** Utilities ***********************//
 public static void change(int arr[] ,int randA[] , int randB[] ,int n)throws Exception{
	int rounds = 1000;
	//for(int i=1;i<=1000;i++){
	  ///int a = 1+((int)(Math.random()*n))%(n-1);
	  //int b = a+((int)(Math.random()*n))%(n-a);
	  for(int i=1;i<=n;i++){
	  int a = randA[i];
	  int b = randB[i];
