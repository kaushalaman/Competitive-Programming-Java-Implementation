	...
	*/
	...
	public static void main(String[] args) throws java.lang.Exception{
		...
		for(int t=1;t<=tests;t++){
			...
			LinkedList<Integer> st[] = new LinkedList[2005];
			LinkedList<Integer> end[] = new LinkedList[2005];
			for(int i=0;i<=2001;i++){st[i]=new LinkedList<Integer>();end[i]=new LinkedList<Integer>();}
			for(int i=0;i<=100004;i++)finish[i]=0;
			for(int i=1;i<=n;i++){
			...
			for(int i=0;i<=2001;i++){
			  ...
			  if(end[i].size()!=0){
			    int check=0;
				while(end[i].size()!=0){
				 int e = end[i].removeFirst();
				 if(finish[e]!=1)check=1;
				 finish[e]=1;
				}
			    if(ll.size()!=0&&check==1){
					ans++;
					while(ll.size()!=0)finish[ll.removeLast()]=1;	
				}
