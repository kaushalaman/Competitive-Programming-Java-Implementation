...
public static void main(String[] args) throws java.lang.Exception{
		//let_me_start 
	int tests = i(); int ans =0;
	int[] m = new int[3*100000+1];
	for(int t=1;t<=tests;t++){
	  int n = i(); int q = i();
	  for(int i=1;i<=3*n;i++)m[i]=1;
	  int num[] = is(n);
	  //build tree
	  ...
	  for(int Q= 1;Q <= q;Q++){
	   int l=i(); int r = i();
	   if(l-1<=0)ans = query(1,1,n,m,num,r+1,n);
	   else if(r+1>n)ans=query(1,1,n,m,num,1,l-1);
	   else {
	   int g1 = query(1,1,n,m,num,1,l-1);
	   int g2 = query(1,1,n,m,num,r+1,n);
	   ans  = gcd(g1,g2);
	   }
	   out.write(""+ans+"\n");
