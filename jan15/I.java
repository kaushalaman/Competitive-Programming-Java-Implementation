...
public static void main(String[] args) throws java.lang.Exception{
		//let_me_start 
		 ...
		 //String ans = "No";
        long ans=0;
        ArrayList <Integer> pf[] = new ArrayList[1001];
		for(int i=1;i<=1000;i++)pf[i]=new ArrayList<Integer>();
		pf[1].add(1);
		...
		for(int t=1;t<=tests;t++){
		    ans=0;
			long n=l();long m = l();int l = i();int r = i();
			for(int i=l;i<=r;i++){
			 long num = n;
			 long tempans=1;
			 for(int j=0;j<pf[i].size();j++){
			  int x = pf[i].get(j);
			  tempans = (((tempans * (m/x))%mod )* num )%mod;
			  num--;
			 }
			tempans = (tempans * pow(m,num,mod))%mod;
			ans+=tempans;
			}
