...
public static void main(String[] args) throws java.lang.Exception{
        ...
        for(int t=1;t<=tests;t++){
			...
			long pans = 0L ,nans = 0L;
			for(int i=1;i<=n-1;i++)nans= (nans + (po[n-i]-1)*arr[i])%mod;
			for(int i=2;i<=n;i++)pans= (pans + (po[i-1]-1)*arr[i])%mod;
			long ans = (pans - nans + mod)% mod;
			out.write(""+ans+"\n");
		 }
