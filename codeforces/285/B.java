...
{	
	...
	*/
	...
	public static long mod = 1000000000+7;
		...
		for(int i=1;i<=n;i++){
		  for(int j=i+1;j<=n;j++){
		     if(a[j].equals("-1"))continue;
		     if(b[i].equals(a[j])){
			   b[i]=new String(b[j].toString());
			   a[j]="-1";
			 }
		  }
		}
		for(int i=1;i<=n;i++)if(!a[i].equals(b[i]) && !a[i].equals("-1"))ans++;
		out.write(""+ans+"\n");
		for(int i=1;i<=n;i++)if(!a[i].equals(b[i]) && !a[i].equals("-1"))out.write(""+a[i]+" "+b[i]+"\n");
