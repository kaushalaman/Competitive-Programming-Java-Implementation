...
{	
	...
	*/
	...
	public static long mod = 1000000000+7;
		...
		long low =1; long hi = 1<<h;
		ans =f(1,1<<h,k,0);//0 1 2
		out.write(""+ans+"\n");
		out.flush();
		return;
		}
...
//****************************** Utilities ***********************//
public static long f(long l , long r ,long k,int turn)throws Exception{
 long mid = (l+r)/2;
 long ans=0;
 if(l>=r)return 1;
 if(turn==0){
  if(l<=k&&k<=mid){
   ans+= 1+f(((mid+l)/2)+1,mid,k,1);
   }else{
   //ans+=mid-l+1;
   long gl = mid-l+1;
   while(gl>=1){ans+=gl;gl/=2;}
