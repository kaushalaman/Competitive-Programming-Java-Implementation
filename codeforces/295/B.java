...
{	
    ...
    */ 
		...
		LinkedList<Long> level = new LinkedList<Long>();
		vis[(int)n]=true;
		queue.add(n);
		level.add(0L);
		while(!queue.isEmpty()){
			long v = queue.removeFirst();
			long l = level.removeFirst();
		if(2*v==m){ans = l+1;break;}
			if(2*v <=200000 && !vis[(int)(2*v)]){
			vis[(int)(2*v)] = true;
			queue.add(2*v);
			level.add(l+1);
			}
			if(v>1){
				if(v-1==m){ans = l+1;break;}
				    if(!vis[(int)v-1]){
					vis[(int)(v-1)] = true;
					queue.add(v-1);
			        level.add(l+1);
...
//****************************** Utilities ***********************//
 ...
 public static boolean po(long n,long m)throws Exception{
  if(n<0)return false;
  while(n<m){
 ...
 public static long cost(long n,long m)throws Exception{
