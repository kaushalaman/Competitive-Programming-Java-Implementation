...
{	
	...
	*/
	public static int tempints[] = new int[500005];
	public static long templongs[] = new long[500005];
	public static double tempdoubles[] = new double[500005];
	public static char tempchars[] = new char[500005];
	public static long mod = 1000000000+7;
		...
		int n = i(); int m = i();int k = i();
		 ...
		 for(int i=1;i<=m;i++){
		   ...
		   long tmp = Math.max(0,(pos[or[i]]-1)/k);
		   ans += 1+tmp;
		  // ans+= Math.max(0,(long)pos[or[i]] - ((tmp)*k) -1);
		       pos[or[i]]-=1;
			   pos[id[pos[or[i]]]]+=1;
		       id[pos[or[i]]+1]=id[pos[or[i]]];
			   id[pos[or[i]]]=or[i];
