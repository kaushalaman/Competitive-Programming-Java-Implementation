...
{	
    ...
    */  //BufferedWriter out=new BufferedWriter(new FileWriter("output.txt"));
		...
		int n = i(); //int m = i();
		 //int arr[] = is((int)n);
		 //String ans = "NO";
		 long ans=0;int idx =1;
		 int arr[] = new int[1000005];
		 while(n>0){
			 arr[idx] = BinarySearch(n) ;
			 n-=arr[idx];
			 idx++;
		 }
		 out.flush();
...
//****************************** Utilities ***********************//
public static int BinarySearch(int n)throws Exception
{
    int m = 0;
    int l = 1; int r = 2000000;
    int ret = 1;
	while( l <= r )
    {
        m = l + (r-l)/2;
        ...
        if( m == n && isq(m)) // first comparison
            return m;
		else if( m == n && isq(m)==false)return ret;
        if( m < n ){ // second comparison
            l = m + 1;
			if(isq(l + (r-l)/2))ret = l;
		}
        else if(m > n){
 ...
 public static boolean isq(int n)throws Exception{
	// String s = ""+n;
	 while(n>0){
