	...
	*/
	...
	public static long mod = 1000000000+7;
		...
		//let_me_start 
		 ...
		 if(arr[(int)n]==arr[1]){
		    temp+=1;
		    if(max<temp){id=1;pre=arr[1];}
			if(max==temp&&arr[1]<pre){id=1;pre=arr[1];}
		 ...
		 for(int i=2;i<=n;i++){
		  if(arr[i]==arr[i-1]){
		    temp+=1;
		    if(max<temp){id=i;pre=arr[i];}
			if(max==temp&&arr[i]<pre){id=i;pre=arr[i];}
			max=Math.max(max,temp);
		 }else{
		  temp=1;
		  
		  max=Math.max(max,temp);
		  if(max==temp&&arr[i]<pre){
		 ...
		 int st= id-max+1;
		  if(st<=0)st+=n;
		 out.write("st="+st+"id="+id+"\n");
		 for(int i=st;;i++){
		  if(i>n)i=1;
		
		  if(i==st&&ch==true)break;
		  out.write(""+(arr[i]+(10-arr[id]))%10);
		  ch=true;
		   }
