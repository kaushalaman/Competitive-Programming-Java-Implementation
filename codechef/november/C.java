...
{	
	...
	*/
	...
	public static void main(String[] args) throws java.lang.Exception{
		//let_me_start 
		double po [] =new double[60];
		po[0] = 1.0D;
		for(int i=1 ; i<58 ;i++)po[i]=2*po[i-1];
		 long tests = l(); 
		 for(int t = 1; t<= tests ;t++){
			double seg = d(); long steps = l();
			double seg_size = seg ; long tot_steps = 0; long offset_steps = 0;
			int i=0;
			double ans =0.0;
			
			while(tot_steps +po[i] <= steps){
			 tot_steps += po[i];
			 
			 if(tot_steps == steps){ 
			   ans = po[i]*seg_size - seg_size/2;  
			 }else{
			   ans = (steps-tot_steps)*(seg_size/2) - (seg_size/2)/2; 
			 }
			 seg_size /= 2.0; 
			 i++;
			}
