...
public static void main(String[] args) throws java.lang.Exception{
		...
		//for(int t=1;t<=tests;t++){
		    int n=i(); int a=i(); int b=i();
			//int arr1 = is(n);
			int pos[]= new int[100005];
			 Pair pair[]= new Pair[2*n+5]; 
//int arr[] =new int[2*n+5];
				
		for(int i=1;i<=n;i++){
			 pair1[i]=new Pair(i(),i);
			}
			
		for(int i=1;i<=n;i++){
			 pair2[i]=new Pair(i(),i);
			}
			Arrays.sort(pair1,1,n+1);
			
			Arrays.sort(pair2,1,n+1);
			//int t=a+b;
			for(int i=n;i>=1;i--){
			 if(pair1[i].a>pair2[i].a){
			  if(a>0 && vis[pair1[i].b]!=1){
			   ans+= pair1[i].a ;
			   vis[pair1[i].b]=1;;
			   a--;
			  }else{
			   if(b>0 && vis[pair2[i].b]!=1){
			   ans+= pair2[i].a ;
			   vis[pair2[i].b]=1;;
			   a--;
			   }
