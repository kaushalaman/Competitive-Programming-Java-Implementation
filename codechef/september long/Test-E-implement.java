...
{	
	...
	*/
	...
	{
		//let_me_start 
		 int px[] = new int[1000005];
		 int nx[] = new int[1000005];
		 for(int i=0;i<=1000004;i++){px[i]=nx[i]=100000005;}
		 in = new Reader();
		 ...
		 for(int i=0;i<n;i++){
		  ...
		  //int x = i(); int y = i();
		 /* if(hm.get(point[i].a)==null){
		    hm.put(point[i].a,new HashMap<Integer,Integer>());
			hm.get(point[i].a).put(point[i].b,1);
		  }else{
		    hm.get(point[i].a).put(point[i].b,1);
		  }
		*/
		 if(point[i].a>=0)px[point[i].a]=point[i].b;
		  else nx[-point[i].a]=point[i].b;
		 }
		 int ans = 2 ;
		// ans = hm.get(2).get(2);
		  
		 Pair p1,p2,p3,p4 ;int a=0,b=0;
		 int x1=0,x2=0,x3=0,x4=0,y1=0,y2=0,y3=0,y4=0,a1=0,a2=0,a3=0,a4=0;
		 for(int i=0;i<n ;i++){
		   for(int j=i+1;j<n;j++){
				//tan(@)=-b/a;
				 b = point[i].a - point[j].a ;
				 a = point[j].b - point[i].b ;
				     x1 = point[i].a + point[j].b-point[i].b ; y1 = point[i].b + point[i].a - point[j].a;
					 x2 = point[j].a + point[j].b-point[i].b ; y2 = point[j].b + point[i].a - point[j].a;
					 x3 = point[i].a - point[j].b+point[i].b ; y3 = point[i].b - point[i].a + point[j].a;
					 x4 = point[j].a - point[j].b+point[i].b ; y4 = point[j].b - point[i].a + point[j].a;
