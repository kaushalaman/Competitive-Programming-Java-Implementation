...
public static void main(String[] args) throws java.lang.Exception{
		//let_me_start 
		 ...
		 //String ans = "No";
        //long ans=0;		
		double [][] p = new double[1002][1002];
		double [][] ans = new double[1002][1002];
		ans[1][1]=p[1][1]=1.0;
		int nn=1000;int mm= 1000;
		for(int i=1;i<=1000;i++){
		  for(int j=1;j<=1000;j++){
				...
				if(j+1<=mm)way++;
				if(way==0)continue;
				if(i+1<=nn)p[i+1][j]+=p[i][j]/way;
				if(j+1<=mm)p[i][j+1]+=p[i][j]/way;
			    ans[i][j] = p[i][j]+p[i-1][j]+p[i][j-1]-p[i-1][j-1];
			  }
			}
